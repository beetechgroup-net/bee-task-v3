...
export function AppShell() {
  ...
  const desktopTasksDropdownRef = useRef<HTMLDivElement>(null);
  const mobileTasksDropdownRef = useRef<HTMLDivElement>(null);
  const desktopTeamDropdownRef = useRef<HTMLDivElement>(null);
  ...
  const isOrgOnboardingRoute =
    ...
    location.pathname === "/organizations/join";

  useEffect(() => {
    if (isAuthenticated && user && user.organizations.length === 0 && !isOrgOnboardingRoute) {
      setShowOrgModal(true);
      return;
    }

    setShowOrgModal(false);
  }, [isAuthenticated, isOrgOnboardingRoute, user]);

  useEffect(() => {
    const handlePointerDown = (event: MouseEvent | TouchEvent) => {
      const target = event.target as Node;
      const isInside = (refs: Array<React.RefObject<HTMLDivElement | null>>) =>
        refs.some((ref) => ref.current?.contains(target));

      if (showOrgSwitcher && orgSwitcherRef.current && !orgSwitcherRef.current.contains(target)) {
        setShowOrgSwitcher(false);
      }

      if (
        showTasksDropdown &&
        !isInside([desktopTasksDropdownRef, mobileTasksDropdownRef])
      ) {
      ...
      if (
        showTeamDropdown &&
        !isInside([desktopTeamDropdownRef, mobileTeamDropdownRef])
      ) {
        setShowTeamDropdown(false);
      }

      if (showAvatarDropdown && avatarDropdownRef.current && !avatarDropdownRef.current.contains(target)) {
  ...
  }, [showAvatarDropdown, showOrgSwitcher, showTasksDropdown, showTeamDropdown]);
    ...
    <div className="flex min-h-screen flex-col bg-app-bg font-sans text-text-main">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-10">
          <div className="flex items-center gap-8">
            ...
            <nav className="hidden items-center gap-1 md:flex">
              ...
              {/* Tarefas Dropdown */}
              <div ref={desktopTasksDropdownRef} className="relative">
                <NavLink
                  to="/"
                  className={({ isActive }) => {
              ...
              {/* Equipe Dropdown */}
              <div ref={desktopTeamDropdownRef} className="relative">
                <NavLink
                  to="/organizations"
                  className={({ isActive }) => {
      ...
      <footer className="border-t border-border-soft bg-surface py-6 md:hidden">
        <nav className="flex justify-around px-4">
          ...
          <div ref={mobileTasksDropdownRef} className="relative">
            <button
          ...
          {/* Mobile Equipe Group */}
          <div ref={mobileTeamDropdownRef} className="relative">
            <button
